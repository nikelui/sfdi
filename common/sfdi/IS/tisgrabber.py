...
# ############################################################################
    GetPropertyAbsoluteValueRange = __tisgrabber.IC_GetPropertyAbsoluteValueRange
    GetPropertyAbsoluteValueRange.restype = C.c_int
    GetPropertyAbsoluteValueRange.argtypes = (GrabberHandlePtr,
                            C.c_char_p,
                            C.c_char_p,
                            C.POINTER(C.c_float),
                            C.POINTER(C.c_float), )
...
class TIS_CAM(object):
        ...
        def GetPropertyAbsoluteValueRange(self, Property, Element, pmin, pmax):
            """ Get a property value range of absolute values interface, e.g. seconds or dB.
            Example code:
            pmin = [0]
            pmax = [0]
            Camera.GetPropertyAbsoluteValue("Exposure","Value", pmin, pmax)
            
            :param Property: Name of the property, e.g. Gain, Exposure
            :param Element: Name of the element, e.g. "Value"
            :param Value: Object, that receives the value of the property
            :param pmin: receives the minimum value of property
            :param pmax: receives the maximum value of property
            :returns: 0 on success
            """
            lmin = C.c_float()
            lmax = C.c_float()
            error = TIS_GrabberDLL.GetPropertyAbsoluteValueRange(self._handle,
                                                    self.s(Property),
                                                    self.s(Element),
                                                    lmin, lmax)
            pmin[0] = lmin.value
